...
def isolate_flare_day(t, f, istart, istop, ipeak, fmask):
    ...
    for sp in range(len(split)):
        if (istart >= prevsplit) & (istart < split[sp]):
            ...
            onday_ipeak = ipeak - split[sp]
        else:
            prevsplit = split[sp]

    onday_fmask = fmask[onday_ind[0]:onday_ind[1]]

    # max(onday_istart - 5, 0)
    # min(onday_istop - 1, len(t) - 1)] #+ buffer
    onday_fmask[max(onday_istart - 5, 0):onday_istop] = 1
    ...
    peak_index = onday_ipeak - onday_istart
    # peak_index = np.argmax(flare_f[:10])

    return onday_t, onday_f, flare_t, flare_f, peak_index, onday_istart, onday_istop, onday_ipeak, onday_fmask
